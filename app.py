...
def _format_actualizar_api_response(result: dict) -> tuple[dict, int]:
    ...
    if result.get("ok"):
        ...
        if result.get("cache") or result.get("cache_used") or result.get("from_json_cache"):
            mensaje = raw_msg or "Mostrando resultados guardados (caché local)."
            base.update({
                "ok": True,
                "warning": True,
                "cache": True,
        ...
        if fuente in ("lotteryusa", "lotterypost") or (
            result.get("warning") and fuente in ("lotteryusa", "lotterypost")
        ):
            labels = {"lotteryusa": "LotteryUSA", "lotterypost": "LotteryPost"}
            label = labels.get(fuente, fuente)
            mensaje = raw_msg or f"Fuente principal falló. Se usó {label}."
            base.update({
                "ok": True,
                "warning": True,
                "fuente": fuente,
...
@login_required
def api_resultados_debug():
    """Diagnóstico fuentes USA (Render/local). Solo módulo USA."""
    if not current_user.is_admin():
        return jsonify({
            "ok": False,
            "error": "Solo administradores pueden ver el debug de resultados.",
        }), 403
    probe = request.args.get("probe", "1").strip().lower() not in ("0", "false", "no")
    from services.usa_results_debug import build_usa_debug_report
...
def _verify_api_routes_registered() -> None:
    ...
    required = (
        "/api/resultados/actualizar",
        "/api/resultados/actualizar-ahora",
        "/api/resultados/debug",
    )
