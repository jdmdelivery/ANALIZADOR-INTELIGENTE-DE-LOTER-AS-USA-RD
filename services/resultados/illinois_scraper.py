...
class IllinoisResultsHubScraper:
    ...
    def _fetch_url(self, url: str) -> dict:
        ...
        markers = ("results-container",) if "results-hub" in url else ("results-content", "main-ball")
        page = fetch_url(
            url,
            ...
            min_bytes=1000 if "results-hub" in url else 500,
        )
        if page.get("ok"):
            save_hub_cache(page["html"], url=page.get("url", url), status_code=page.get("status_code"))
            page["from_cache"] = False
            return page
        page.setdefault("from_cache", False)
        return page
