...
def import_illinois_lottery_now(lottery_name):
    """Importa/actualiza desde Results Hub solo para una lotería Illinois (hoy)."""
    ...
    rows = [
        r for r in parse_results_hub_html(page["html"])
        if r.get("lottery_name", "").lower() == lottery_name.lower()
        and r.get("draw_date") == today_iso
    ]

    lotteries = get_all_lotteries()
    lottery_id = None
    for lot in lotteries:
        if lot.get("country") == "USA" and lot["name"].lower() == lottery_name.lower():
            lottery_id = lot["id"]
            break
    if not lottery_id:
        return {"ok": False, "message": f"Lotería Illinois '{lottery_name}' no encontrada."}
